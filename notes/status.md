...
- **ElevenLabs curated voice list**: добавленные admin-голоса сохраняются в `curated_tts_voices`, доступны через `GET /api/admin/tts/voices`, удаляются через `DELETE /api/admin/tts/voices/{voice_id}` и подмешиваются в `/api/tts/options`, поэтому несколько голосов больше не вытесняют друг друга одним `audio_voice`.
